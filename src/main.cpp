...
int main(int argc, char** argv)
{
    for (uint32_t noun = 0; noun < 100; ++noun)
    {
        for (uint32_t verb = 0; verb < 100; ++verb)
        {
            ...
            // @TODO: The result should be in position 0, not 1...
            if (pMemory[1] == 19690720)
            {
                printf("Solution Found: Noun -> %u, Verb -> %u\n", noun, verb);
            ...
            {
                printf("Solution Invalid: %" PRIu64 "\n", pMemory[1]);
            }
        }
    }
